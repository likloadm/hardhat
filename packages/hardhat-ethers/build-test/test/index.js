...
describe("Ethers plugin", function () {
        ...
        describe("Signers and contracts helpers", function () {
            ...
            describe("getImpersonatedSigner", function () {
                it("should invoke hardhat_impersonateAccount", async function () {
                    ...
                    await chai_1.assert.isRejected(this.env.ethers.getImpersonatedSigner(address), "Method hardhat_impersonateAccount not supported");
                });
            ...
            describe("getContractAt", function () {
                ...
                describe("by name and address", function () {
                    it("Should throw if address does not belong to a contract", async function () {
                        ...
                        return chai_1.assert.isRejected(this.env.ethers.getContractAt("Greeter", address), `${address} is not a contract account.`);
                    });
            ...
            describe("deployContract", function () {
                ...
                it("should deploy and return a contract with custom signer passed directly", async function () {
                    const contract = await this.env.ethers.deployContract("Greeter", signers[1]);
                ...
                it("should deploy with args and return a contract", async function () {
                    const contract = await this.env.ethers.deployContract("GreeterWithConstructorArg", ["Hello"]);
                    await assertContract(contract, signers[0]);
                    (0, chai_1.assert)(await contract.greet(), "Hello");
                });
                it("should deploy with args and return a contract with custom signer", async function () {
                    const contract = await this.env.ethers.deployContract("GreeterWithConstructorArg", ["Hello"], signers[1]);
                    await assertContract(contract, signers[1]);
                    (0, chai_1.assert)(await contract.greet(), "Hello");
                });
                it("should deploy with args and return a contract with custom signer as an option", async function () {
                    const contract = await this.env.ethers.deployContract("GreeterWithConstructorArg", ["Hello"], { signer: signers[1] });
                    await assertContract(contract, signers[1]);
                    (0, chai_1.assert)(await contract.greet(), "Hello");
                });
                async function assertContract(contract, signer) {
                    chai_1.assert.containsAllKeys(contract.interface.functions, [
                        "setGreeting(string)",
                        "greet()",
                    ]);
                    chai_1.assert.equal(await contract.signer.getAddress(), await signer.getAddress());
                }
            });
        });
    });
    ...
    describe("ganache via WebSocket", function () {
        ...
        it("should be able to detect events", async function () {
            ...
            const Greeter = await this.env.ethers.getContractFactory("Greeter");
            const deployedGreeter = await Greeter.deploy();
            const readonlyContract = deployedGreeter.connect(new ethers_1.ethers.providers.WebSocketProvider("ws://127.0.0.1:8545"));
            let emitted = false;
