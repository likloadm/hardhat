...
const qtum_ethers_wrapper_1 = require("qtum-ethers-wrapper");
...
class LocalAccountsProvider extends chainId_1.ProviderWrapperWithChainId {
    ...
    async request(args) {
        const { ecsign, hashPersonalMessage, toRpcSig, toBuffer, bufferToHex } = await Promise.resolve().then(() => __importStar(require("@nomicfoundation/ethereumjs-util")));
    ...
    _initializePrivateKeys(localAccountsHexPrivateKeys) {
        const { bufferToHex, toBuffer, privateToAddress, } = require("likloadm-ethereumjs-util");
    ...
    async _getNonce(address) {
        const { bufferToHex } = await Promise.resolve().then(() => __importStar(require("@nomicfoundation/ethereumjs-util")));
    ...
    async _getSignedTransaction(transactionRequest, chainId, privateKey) {
        const { AccessListEIP2930Transaction, Transaction, TransactionFactory } = await Promise.resolve().then(() => __importStar(require("@nomicfoundation/ethereumjs-tx")));
        const { bufferToHex, toBuffer, privateToAddress, } = require("likloadm-ethereumjs-util");
        ...
        const qtumProvider = new qtum_ethers_wrapper_1.QtumFunctionProvider(async (method, params) => {
        ...
        const accessList = txData.accessList?.map(({ address, storageKeys }) => [address.toString(), storageKeys]);
        let transaction;
        ...
        const qtumWallet = new qtum_ethers_wrapper_1.QtumWallet(privateKey, qtumProvider, { filterDust: false });
...
class HDWalletProvider extends LocalAccountsProvider {
    constructor(provider, mnemonic, hdpath = "m/44'/60'/0'/0/", initialIndex = 0, count = 10, passphrase = "") {
        const privateKeys = (0, util_1.derivePrivateKeys)(mnemonic, hdpath, initialIndex, count, passphrase);
        const { bufferToHex } = require("@nomicfoundation/ethereumjs-util");
